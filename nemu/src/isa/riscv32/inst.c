...
/*
int64_t mul_temp;
static int get_bits(int64_t num)
{
	int count = 0;
	int64_t temp = num;
	while(temp){count++; temp = temp / 2;}
	return count;
}*/
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));
	...
	INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I,  s->dnpc = (src1 + imm) & ~(uint32_t)0x1; R(rd) = s->snpc; IFDEF(CONFIG_FTRACE, fun_state = rd != 0 ? CALL : RET));
	...
	INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq		 , B, s->dnpc = ((sword_t)src1) == ((sword_t)src2) ? s->pc + imm : s->dnpc);
	...
	INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div		 , R, R(rd) = (sword_t)((sword_t)src1 / (sword_t)src2));
	INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem		 , R, R(rd) = (sword_t)((sword_t)src1 % (sword_t)src2));
