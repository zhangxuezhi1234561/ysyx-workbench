...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));
	...
	INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal		 , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm; fun_state = CALL);
	INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, temp = s->pc + 4;  s->dnpc = (src1 + imm) & ~1; R(rd) = temp; fun_state = rd != 0 ? CALL : RET);
